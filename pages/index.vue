<template>
  <div class="page">
    <div class="content">
      <div class="page-columns inner-left">
        ...
        <pw-section class="blue" label="Request" ref="request">
          <ul>
            ...
            <li>
              <label class="hide-on-small-screen" for="stopSend">&nbsp;</label>
              <button
                :disabled="!isValidURL"
                @click="handleCancelTaskTime"
                id="stopSend"
                ref="stopSendButton"
                v-tooltip.bottom="{
                  content: 'Timer Duration:  ' + taskTimeoutSeconds + ' ms'
                }"
              >
                {{ $t("Stop") }}
                <span>
          ...
          <div class="flex-wrap">
            <span>
              ...
              <button
                ...
                id="taskTimeButton"
                v-tooltip.bottom="{
                  content: 'Setting Timer'
                }"
                @click="showTaskTime = !showTaskTime"
              >
                <i
                  class="material-icons"
                  :class="showPreRequestScript"
      ...
      <pw-modal v-if="showTaskTime" @close="showTaskTime = false">
        <div slot="header">
          <ul>
            <li>
              <div class="flex-wrap">
                <h3 class="title">{{ $t("Setting Timer") }}</h3>
                <div>
                  <button class="icon" @click="showTaskTime = false">
                    <i class="material-icons">close</i>
                  </button>
                </div>
              </div>
        ...
        <div slot="footer">
          <div class="flex-wrap">
            ...
            <span>
              ...
              <button class="icon primary" @click="handleSetting">
                {{ $t("Setting") }}
              </button>
            </span>
          </div>
    ...
    methods: {
      ...
      async sendRequest() {
        ...
        if (this.hasRequestBody) {
          this.tempRequestBody = this.rawInput ? this.rawParams : this.rawRequestBody;
        ...
        this.tempHeaders = headersObject;

        if (this.taskTimeoutSeconds) {
          this.taskTime = setInterval(this.send, this.taskTimeoutSeconds)
        } else {
          this.send()
        }
      },
      ...
      observeRequestButton() {
        ...
        const stopSendButtonElement = this.$refs.stopSendButton;
        const observer = new IntersectionObserver(
          (entries, observer) => {
            entries.forEach(entry => {
              if (entry.isIntersecting) {
                sendButtonElement.classList.remove("show");
                stopSendButtonElement.classList.remove("show");
              }
                // The button should float when it is no longer visible on screen.
              // This is done by adding the show class to the button.
              else {
                sendButtonElement.classList.add("show");
                stopSendButtonElement.classList.add("show");
              }
            });
          },
          {
      ...
      handleSetting() {
        let input = document.getElementById("task-time")
        this.taskTimeoutSeconds = Number(input.value);
        this.showTaskTime = false
      },
      handleCancelTaskTime() {
